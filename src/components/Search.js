                    ...
                    (<ul className="absolute searchModal inset-0 top-[3.5rem] md:left-3 lg:left-4 w-full max-h-80 lg:w-[38.5%] h-max rounded-md bg-[#414141] overflow-x-hidden  "  >
                        {
                            searchData ? searchData.map((data, key) => (
                                <button onClick={(e) => searchClearer(data.id)} key={key} className="hover:bg-purple-900 flex items-center justify-between space-x-4 md:space-x-6 w-full h-max py-1 px-3 md:px-5 md:py-2 lg:px-6 lg:py-3 " >
                                    <img src={data.large} alt={data.name} className="h-5 w-5 md:h-5 md:w-5 lg:h-6 lg:w-6 lg:basis-[7%] " />
                                    <p className="font-semibold basis-[70%] text-start " >{data.name}</p>
...
const Search = () => {
    const { getSearchData, setCurrency, setOrder } = useContext(ListContext)
    ...
    return (
        <div className="flex flex-col w-[85%] md:w-[80%] mx-auto lg:flex-row p-1 sm:p-2 md:p-3 lg:p-4 justify-between items-center relative">
            <SearchInput searchRefresh={searchRefresh} />
            <div className="flex w-max lg:space-x-12 " >
                <div className="flex items-center justify-center space-x-4 md:space-x-8 lg:space-x-3" >
                    <div className="" >Currency</div>
                    <select onChange={(e) => setCurrency(e.target.value)} className="bg-[#313131] uppercase rounded-md h-full w-full px-1 md:px-2 lg:px-3 py-1 md:py-2 outline-none " >
                        {
                            curr.map((cur, key) => {
                ...
                <div className="flex items-center justify-center space-x-4 md:space-x-8 lg:space-x-3" >
                    ...
                    <select onChange={(e) => setOrder(e.target.value)} className="bg-[#313131] rounded-md h-full w-full px-1 md:px-2 lg:px-3 py-1 md:py-2 outline-none " >
                        {
                            ord.map((cur, key) => {
                                return (<option value={cur} >{cur}</option>)
                            }
